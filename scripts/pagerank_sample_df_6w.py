...
def main():
    ...
    # CONFIG
    # -----------------------------
    input_path = "gs://pagerank-marah-wiki/preproc/sample_10pct.parquet"   
    output_path = "gs://pagerank-marah-wiki/results/df_sample_6w_results"
    top10_output = "gs://pagerank-marah-wiki/results/top10_sample_df_6w"
    ...
    print("✔ PageRank terminé et sauvegardé.")
...
spark = SparkSession.builder.appName("Top10-PageRank").getOrCreate()

# Load previously computed PageRank results
ranks = spark.read.parquet("gs://pagerank-marah-wiki/results/df_results")

# Get the top 10
top10 = ranks.orderBy(col("rank").desc()).limit(10)

top10.show(10, truncate=False)

# Optionally save them
top10.write.mode("overwrite").parquet("gs://pagerank-marah-wiki/results/top10_df")
